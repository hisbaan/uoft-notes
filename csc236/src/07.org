...
Use subset construction, notes page 219 if \(\Sigma = \cbr{0, 1}\), the construction is, roughly:
...
This system lets us deal with the creation of determinsitic finite state automata for regular expressions much more easily by making it a non-deterministic finite state automata and then converting it into a deterministic one.
