...
Let \(e_{1}, e_{2} \in \cE\), and assume \(P(e_{1})\) and \(P(e_{2})\). Want to prove that \(P(\br{e_{1} \odot e_{2}})\) where \(\odot \in \cbr{+, -, \times, \div}\), that is \(\vr{\cbr{e_{1} \odot e_{2}}} = \op{\cbr{e_{1} \odot e_{2}}} + 1\).
\begin{align*}
  \vr{\br{e_{1} \odot e_{2}}} &= \vr{e_{1}} + \vr{e_{2}} \\
                          &= \op{e_{1}} + 1 + \op{e_{2}} + 1 && \text{(by \(P(e_{1})\) and \(P(e_{2})\))} \\
                          &= \sbr{\op{e_{1}} + 1 \op{e_{2}}} + 1 \\
                          &= \op{e_{1} \odot e_{2}} + 1
\end{align*}
...
% TODO add the last example from the annotated slided later
\end{align*}
