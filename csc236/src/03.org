...
/Proof:/ For every \(e \in \cE\) define \(P(e)\) as \(\vr{e} \leq 2^{h(e)}\). Let \(a \in \cbr{x, y, z}\). Then \(a\) has one variable (itself), and no operators so \(\vr{a} = 1 = 2^{0} = 2^{h(a)}\) since \(h\) of any single vairable is 0. So \(P(a)\) holds for \(a \in \cbr{x, y, z}\) (the basis).
