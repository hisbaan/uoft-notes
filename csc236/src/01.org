#+title: CSC236 Lecture 01: Theory of Computation
...
- collabourate with respect

  You need computerscience friends who are respectful and constructively critical.
...
- undersatnd and use several flavours of induction.
...
If the inital case works, and each case that works implies its successor works, then all cases work
...
\item[\(i\).] Induction on \(n\)
    ...
    Let \(n\) be an arbitary, fixed, natural number. (Let \(n \in \N\)).
