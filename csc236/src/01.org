...
\(P(n)\): a \(2^n \times 2^n\) square grid, with one subsquare removed, can be tiled (covered without overlapping) by "chair" trominoes.
...
# decompose rather than construct. Take a bigger thing and then break it down, don't build up a smaller on because then you know that there will always be a bigger thing. You don't have to guess at the existence of the bigger thing you're making. Except in structural induction (later)
