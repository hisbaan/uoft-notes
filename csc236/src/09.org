...
- A simil arguemnt implies irregularty of \(L'' \cbr{x \in \Sigma^* : x \text{ has an equal number of }}\)
...
- If we try and caluclate this, we will find that it is impossible. We cannot calculate how many states there are in a given machine using that same machine --- we will run out of ram.

* Push Down Automata (PDA)

- DFSA plus an infinite stack with finite set of stack symbols. Each transtition depends on the state, (optionally) the input symbol, (optionally) a pop from stack.
- Each transition results in a state, (optionnal)  push onto stack.

Design a PDA that accepts \(L = \cbr{1^n0^n : n \in \N}\).

Corresponds to a context-free gramma (production rules) that denotes this language
- \(S \to 1S0\)
- \(S \to \varepsilon\)
...
- (informally) turing machine: finite states, read/write an infinte tape of memory, tape moves are one position L-to-R.
