...
Quintuple: \(\br{Q, \Sigma, q_0, F,\delta}\)
...
\[
  \delta^{*} : Q \times \Sigma^{*} \mapsto Q \qquad \delta^{*}(q, s) = \begin{cases} q & \text{ if } s = \varepsilon \\ \delta(\delta^{*}(q, s'), a) & \text{ if } s' \in \Sigma^{*}, a \in \Sigma, s = s'a \end{cases}
\]
