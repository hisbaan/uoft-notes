...
Form some expressions in \(\mathcal{E}\). Count the number of variables (sumbols from \(\{x, y, z\}\)) and the number of operators symbols from \(\{+, -, \times, \div\}\). Make a conjecture
